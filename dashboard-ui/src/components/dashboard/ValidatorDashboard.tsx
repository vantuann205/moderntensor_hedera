...
import { Shield, ExternalLink, RefreshCw, ChevronRight, AlertCircle, CheckCircle, Gift, Unlock, TrendingUp, Star, Gavel } from 'lucide-react';
...
function ScoreSubmissionPanel({ accountId, evmAddress }: { accountId: string; evmAddress: string }) {
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        // Only tasks that have an on-chain taskId (created via createTask)
        const withId = all.filter(t => t.onChainTaskId != null && t.onChainTaskId !== '');
        setTasks(withId.slice(0, 15));

        // Fetch submissions for each task from contract
        ...
        await Promise.all(withId.slice(0, 15).map(async (t) => {
          try {
  ...
  const scoreSubmission = async (task: any, subIdx: number) => {
    const key = `${task.onChainTaskId}_${subIdx}`;
    ...
    try {
      ...
      if (walletType === 'metamask') {
        ...
        const params = new ContractFunctionParameters()
          .addUint256(String(onChainId))
          .addUint256(String(subIdx))
          .addUint256(String(scoreVal));
        ...
        setDone(d => ({ ...d, [key]: String(receipt.transactionId || 'submitted') }));
      }
      await loadTasks();
  ...
  const finalizeTask = async (task: any) => {
    const key = `finalize_${task.onChainTaskId}`;
    setSubmitting(s => ({ ...s, [key]: true })); setErrors(e => ({ ...e, [key]: '' }));
    try {
      if (walletType === 'metamask') {
        const provider = new ethers.BrowserProvider((window as any).ethereum);
        ...
        const hederaId = AccountId.fromString(accountId);
        const contractId = ContractId.fromEvmAddress(0, 0, CONTRACTS.SUBNET_REGISTRY);
        const params = new ContractFunctionParameters().addUint256(String(task.onChainTaskId));
        const receipt = await hashConnect.sendTransaction(hederaId,
          new ContractExecuteTransaction().setContractId(contractId).setGas(400000).setFunction('finalizeTask', params));
        setDone(d => ({ ...d, [key]: String(receipt.transactionId || 'finalized') }));
      }
      await loadTasks();
    ...
    <div className="glass-panel rounded-2xl border border-neon-cyan/20 overflow-hidden">
      ...
      <div className="divide-y divide-white/5 max-h-[600px] overflow-y-auto">
        {loadingTasks ? (
          ...
          const onChainId = task.onChainTaskId;
          const subs = submissions[onChainId] || [];
          const finalizeKey = `finalize_${onChainId}`;
            ...
            <div key={ti} className="px-5 py-4 space-y-3">
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right flex-shrink-0">
                  <div className="text-neon-green font-black text-sm">{(Number(task.rewardAmount ?? 0) > 1e6 ? Number(task.rewardAmount) / 1e8 : Number(task.rewardAmount ?? 0)).toLocaleString()} MDT</div>
                  <div className="text-[9px] text-slate-500">{subs.length} submission{subs.length !== 1 ? 's' : ''}</div>
                </div>
              </div>
                    ...
                    {!sub.validated && (
                      isDone ? (
                        <div className="flex items-center gap-2 text-neon-green text-[10px] font-bold">
                          ...
                          <a href={`https://hashscan.io/testnet/transaction/${done[key]}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-neon-cyan hover:underline ml-1"><ExternalLink size={9} /> HashScan</a>
                        </div>
                      ) : (
                        <div className="flex gap-2">
                          <input type="number" min="0" max="10000" placeholder="Score (0–10000)" value={scores[key] || ''}
                            onChange={e => setScores(sc => ({ ...sc, [key]: e.target.value }))}
                            className="flex-1 bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white font-mono text-xs focus:border-neon-cyan/40 outline-none" />
                          <button onClick={() => scoreSubmission(task, si)} disabled={submitting[key]}
                            className="px-4 py-2 bg-neon-cyan/10 hover:bg-neon-cyan/20 border border-neon-cyan/40 text-neon-cyan font-black text-[10px] uppercase tracking-widest rounded-lg transition-all disabled:opacity-40 flex items-center gap-1.5 whitespace-nowrap">
                ...
                <div className="pt-1">
                  {done[finalizeKey] ? (
                    <div className="flex items-center gap-2 text-neon-green text-[10px] font-bold">
                      ...
                      <a href={`https://hashscan.io/testnet/transaction/${done[finalizeKey]}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-neon-cyan hover:underline ml-1"><ExternalLink size={9} /> HashScan</a>
                    </div>
                  ) : (
...
export default function ValidatorDashboard({ onBack }: Props) {
  ...
  const avgScore = scores.length > 0 ? scores.reduce((a: number, s: any) => a + Number(s.score ?? 0), 0) / scores.length : null;
      ...
      <div className="w-full max-w-[1400px] flex flex-col gap-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Avg Accuracy', val: avgScore !== null ? `${(avgScore * 100).toFixed(1)}%` : '—', icon: 'verified', color: 'text-neon-cyan', sub: `${scores.length} validations` },
          ...
          <div className="xl:col-span-2 flex flex-col gap-6">
            ...
            <div className="glass-panel rounded-2xl border border-white/10 overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
                ...
                {avgScore !== null && <div className="text-right"><div className="text-neon-cyan font-black text-lg">{(avgScore * 100).toFixed(1)}%</div><div className="text-[9px] text-slate-500 uppercase tracking-widest">Avg Accuracy</div></div>}
              </div>
              <div className="divide-y divide-white/5 max-h-[400px] overflow-y-auto">
                {loading ? <div className="py-8 text-center text-slate-600 text-xs font-mono">Loading...</div>
                  : scores.length === 0 ? (
                    ...
                    const sc = score >= 0.9 ? 'text-neon-green' : score >= 0.7 ? 'text-neon-cyan' : 'text-yellow-400';
                    return (
                      <div key={i} className="px-6 py-3 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
                        <div>
                          ...
                          <div className="text-[9px] text-slate-600 mt-0.5">Miner: {s.minerId || s.miner_id || '—'} · {toUTC7(s.consensusTimestamp)}</div>
                        </div>
                        <div className="text-right">
                          <div className={`font-black text-sm ${sc}`}>{(score * 100).toFixed(1)}%</div>
                          <div className="w-16 h-1 bg-white/10 rounded-full overflow-hidden mt-1"><div className={`h-full rounded-full ${sc.replace('text-', 'bg-')}`} style={{ width: `${score * 100}%` }} /></div>
                        </div>
                      </div>
                    );
                  })}
