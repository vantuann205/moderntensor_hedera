      ...
      <div className="w-full max-w-[1400px] flex flex-col gap-8">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="px-3 py-1 bg-neon-pink/10 text-neon-pink text-[12px] font-black border border-neon-pink/30 rounded uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="w-full glass-panel rounded-xl p-2 flex flex-col md:flex-row items-center justify-between gap-4 border border-white/5 bg-panel-dark/40 font-display">
            <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto">
              {['All', 'Transfers', 'Staking', 'Governance'].map((t, i) => (
                <button key={t} className={`px-4 py-2 rounded text-[12px] font-black uppercase tracking-widest transition-all ${i === 0 ? 'bg-neon-pink text-black' : 'bg-white/5 text-slate-500 hover:text-white'}`}>{t}</button>
              ))}
            </div>
            <div className="relative w-full md:w-64">
              <input 
                className="w-full bg-black/40 border border-white/10 rounded px-4 py-2 text-white focus:border-neon-pink outline-none text-[12px] font-mono tracking-wider" 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-[#050b14]/80 backdrop-blur">
                <tr className="border-b border-white/10 text-[12px] uppercase font-black text-slate-500">
