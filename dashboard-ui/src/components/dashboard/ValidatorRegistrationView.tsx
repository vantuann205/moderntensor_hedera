...
import { CONTRACTS, STAKING_VAULT_VALIDATOR_ABI, HTS_ABI, HTS_PRECOMPILE, StakeRole } from '@/lib/contracts';
import { ViewState } from '@/types';
import { Shield, ExternalLink, AlertCircle, CheckCircle, Lock, RefreshCw } from 'lucide-react';
        ...
        {result && (
          <div className="p-4 bg-neon-green/5 border border-neon-green/20 rounded-xl space-y-2">
            <div className="flex items-center gap-2 text-neon-green text-[11px] font-black">
              <CheckCircle size={14} /> {result.amount} MDT sent successfully
            </div>
            {result.txId && (
...
const MIN_VALIDATOR_STAKE = 500;
const REG_FEE_BUFFER = 6;
const HEDERA_RPC = CONTRACTS.HEDERA_RPC;
  ...
  try {
    ...
    if (typeof txId === 'string') {
      if (txId.includes('@')) {
        ...
        mirrorId = `${acc}-${time.slice(0, dotIdx)}-${time.slice(dotIdx + 1)}`;
      } else { mirrorId = txId; }
...
type Step = 'idle' | 'checking' | 'need_faucet' | 'faucet_pending' | 'deposit' | 'staking' | 'hcs' | 'done' | 'error';

export default function ValidatorRegistrationView({ onBack, onViewChange, onRegistered }: Props) {
  ...
  const busy = ['checking', 'faucet_pending', 'deposit', 'staking', 'hcs'].includes(step);
  ...
  const checkBalance = useCallback(async () => {
    ...
    try {
      ...
      const info = { mdtBalance: data.mdtBalance, hbarBalance: data.hbarBalance, evmAddress: data.evmAddress || walletEvm || '', hasEnough: data.mdtBalance >= totalNeeded };
      setBalance(info);
      ...
      const stakeInfo = await vault.getStakeInfo(info.evmAddress);
      if (stakeInfo.isActive) {
        const role = Number(stakeInfo.role);
        if (role === StakeRole.Validator) {
          setAlreadyStaked(true);
          log(`✓ Already staked as Validator (${Number(stakeInfo.amount) / 1e8} MDT)`);
        } else {
          const roleName = role === StakeRole.Miner ? 'Miner' : role === 3 ? 'Holder' : `role ${role}`;
  ...
  useEffect(() => {
    if (isConnected && accountId) checkBalance();
  }, [isConnected, accountId]); // eslint-disable-line

  const requestFaucet = async () => {
    if (!accountId) return;
    setStep('faucet_pending'); setError(null); setLogs([]);
    try {
  ...
  const doStake = async (evmAddress: string): Promise<{ txHash: string; txHashTransfer?: string; txHashContract?: string } | null> => {
    ...
    const stakeRole = StakeRole.Validator; // 2
    ...
    try {
      if (walletType === 'metamask') {
        ...
        const vaultRead = new ethers.Contract(CONTRACTS.STAKING_VAULT, STAKING_VAULT_VALIDATOR_ABI, provider);
        const existing = await vaultRead.getStakeInfo(signerAddr);
        if (existing.isActive && Number(existing.role) === StakeRole.Validator) {
          log(`✓ Already staked as Validator (${Number(existing.amount) / 1e8} MDT)`);
          return { txHash: 'already_staked' };
        }
        if (existing.isActive && Number(existing.role) !== StakeRole.Validator) {
          throw new Error(`Already staked as role ${Number(existing.role) === 1 ? 'Miner' : 'Holder'} — cannot stake as Validator while active. Request unstake first.`);
        }
        ...
        // Step 1: Transfer MDT to vault via HTS precompile
        setStep('deposit');
        ...
        // Step 3: stake()
        setStep('staking');
        log(`Step 3/3: Calling vault.stake(${stakeNum} MDT, Validator) — approve in MetaMask...`);
        ...
        log(`Step 3/3: Calling vault.stake(${stakeNum} MDT, Validator) — approve in HashPack...`);
      ...
      log(`✗ Stake failed: ${e.message}`);
      setError(`Stake failed: ${e.message}`);
      return null;
    }
  };
  ...
  const handleRegister = async () => {
    ...
    if (stakeNum < MIN_VALIDATOR_STAKE) { setError(`Minimum stake is ${MIN_VALIDATOR_STAKE} MDT`); return; }
    ...
    if (alreadyStaked) {
      log(`✓ Already staked as Validator — skipping stake`);
    ...
    // HCS registration
    setStep('hcs');
    log(`Submitting validator_register to HCS topic 0.0.8198583...`);
    try {
      const hcsRes = await fetch('/api/hcs/register', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ role: 'validator', accountId, stakeAmount: stakeNum, skipOnChainStake: true }),
      });
  ...
  const stepLabel = { idle: 'Ready', checking: 'Checking...', need_faucet: 'Insufficient MDT', faucet_pending: 'Requesting MDT...', deposit: 'Recording deposit...', staking: 'Staking on-chain...', hcs: 'Submitting to HCS...', done: 'Registered ✓', error: 'Error' }[step];
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-4 border-b border-white/5 pb-6">
        ...
        <div>
          <h1 className="text-3xl font-display font-bold text-white uppercase tracking-tighter italic">
            Join as <span className="text-neon-purple">Validator</span>
          </h1>
          <p className="text-[12px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
            <span className={`w-1.5 h-1.5 rounded-full ${step === 'done' ? 'bg-neon-green' : step === 'error' ? 'bg-red-400' : 'bg-neon-purple animate-pulse'}`} />
            {stepLabel}
      ...
      {/* Info box */}
      <div className="glass-panel p-4 border border-neon-purple/20 space-y-2 text-[11px] text-slate-400 leading-relaxed">
        <div className="flex items-center gap-2 text-neon-purple font-black text-xs uppercase tracking-widest">
          <Shield size={14} />
          Validator Registration Flow
        </div>
        <div>1. Stake ≥500 MDT as Validator role on <span className="text-white font-mono">StakingVaultV2</span></div>
        <div>2. Submit HCS registration to topic <span className="text-neon-cyan font-mono">0.0.8198583</span></div>
        <div>3. Subnet owner calls <span className="text-white font-mono">addValidator(subnetId, yourAddress)</span> to activate you</div>
        <div className="text-yellow-400 text-[12px] pt-1">⚠ Validator activation requires subnet owner approval — you will see "Pending Approval" after staking.</div>
      </div>
        ...
        <div className={`glass-panel p-4 flex items-center justify-between border ${balance.hasEnough || alreadyStaked ? 'border-neon-green/20' : 'border-yellow-400/30'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[12px] text-slate-500 font-mono">{balance.mdtBalance.toFixed(2)} MDT · {balance.hbarBalance.toFixed(4)} HBAR</div>
            </div>
          </div>
          <div className="text-right">
            ...
            <div className="text-[11px] text-slate-500 uppercase tracking-widest">{alreadyStaked ? 'Already staked as Validator' : 'MDT needed'}</div>
          </div>
        </div>
      )}
        ...
        return (
          <div className="glass-panel p-5 border border-red-500/30 rounded-2xl space-y-3">
            ...
            <p className="text-[11px] text-slate-300 leading-relaxed">
              This wallet is already staked as <span className="text-red-400 font-bold">{conflictRole}</span>. One account = one role — you cannot register as Validator while active as {conflictRole}.
            </p>
            <div className="p-3 bg-yellow-400/5 border border-yellow-400/20 rounded-xl text-[10px] text-yellow-400 space-y-1">
              <div className="font-black uppercase tracking-widest mb-1">How to switch to Validator</div>
              ...
              <div>4. Come back here and register as Validator</div>
            </div>
          </div>
        );
      })()}
        ...
        <div className="glass-panel p-6 space-y-5">
          ...
          <div>
            <label className="text-[12px] font-bold text-slate-500 uppercase tracking-widest block mb-2">
              Stake Amount (MDT) <span className="text-neon-purple">· min {MIN_VALIDATOR_STAKE} MDT</span>
            </label>
            <input type="number" min={MIN_VALIDATOR_STAKE} value={stake}
              onKeyDown={(e) => ["e", "E", "+", "-"].includes(e.key) && e.preventDefault()} onChange={e => setStake(e.target.value)}
              className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-sm font-mono text-white focus:border-neon-purple/40 outline-none" />
            <div className="text-[12px] text-slate-600 mt-1">Total needed: {totalNeeded} MDT (stake + ~{REG_FEE_BUFFER} regFee buffer)</div>
          </div>

          {/* On-chain flow info */}
          <div className="p-3 bg-neon-purple/5 border border-neon-purple/20 rounded-xl text-[12px] text-slate-400 space-y-0.5">
            <span className="text-neon-purple font-black uppercase tracking-widest block mb-1">On-Chain Flow</span>
            {walletType === 'hashpack' ? (<>
              ...
              <div>3. HashPack: <code className="text-neon-cyan">vault.stake({stakeNum * 1e8}, 2 /* Validator */)</code></div>
            </>) : (<>
              <div>1. MetaMask: <code className="text-neon-cyan">HTS cryptoTransfer — {totalNeeded} MDT → vault</code></div>
              <div>2. Backend: <code className="text-neon-cyan">recordDeposit(userEVM, {totalNeeded} MDT)</code></div>
              <div>3. MetaMask: <code className="text-neon-cyan">vault.stake({stakeNum * 1e8}, 2 /* Validator */)</code></div>
            </>)}
            <div>4. HCS: <code className="text-neon-cyan">validator_register → topic 0.0.8198583</code></div>
          </div>

          {error && <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-xl text-xs font-mono text-red-400">✗ {error}</div>}

          {(() => {
            const notReady = !isConnected || stakeNum < MIN_VALIDATOR_STAKE || (!balance?.hasEnough && !alreadyStaked);
            const isDisabled = busy || notReady;
            const btnLabel = busy ? stepLabel
              : !isConnected ? 'Connect wallet first'
              : stakeNum < MIN_VALIDATOR_STAKE ? `Min stake is ${MIN_VALIDATOR_STAKE} MDT`
              : !balance?.hasEnough && !alreadyStaked ? `Need ${totalNeeded} MDT — use faucet ↑`
              : alreadyStaked ? 'Submit HCS Registration'
              : 'Stake & Register as Validator';
            const btnColor = busy ? 'bg-neon-purple/10 border-neon-purple/40 text-neon-purple opacity-70 cursor-wait'
              : isDisabled ? 'bg-slate-800/60 border-slate-700/60 text-slate-500 cursor-not-allowed'
              : 'bg-neon-purple/10 hover:bg-neon-purple/25 border-neon-purple/50 text-neon-purple hover:shadow-[0_0_16px_rgba(168,85,247,0.2)]';
            return (
      ...
      {step === 'done' && hcsResult && (
        <div className="glass-panel p-6 space-y-4 border border-neon-green/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-black uppercase tracking-wider text-neon-green">Staked & Registered on HCS</div>
              <div className="text-[12px] text-slate-500 font-mono mt-0.5">Awaiting subnet owner approval to activate</div>
            </div>
          </div>
          <div className="p-4 bg-yellow-400/5 border border-yellow-400/20 rounded-xl text-[11px] text-yellow-400 space-y-1">
            <div className="font-black uppercase tracking-widest text-xs mb-2">Pending Approval</div>
            <div>Your stake is confirmed. A subnet owner must call <code className="text-white">addValidator(subnetId, {walletEvm || accountId})</code> to activate you as a validator.</div>
            <div className="text-slate-500 mt-1">Contact the subnet owner or protocol team to complete activation.</div>
          </div>
          <div className="grid grid-cols-2 gap-3 font-mono text-xs">
            ...
            <div className="p-3 bg-black/40 rounded-xl border border-white/5 space-y-1">
              <div className="text-slate-500 text-[11px] uppercase tracking-widest">Stake</div>
              <div className="text-white">{stakeNum} MDT</div>
              <div className="text-neon-purple text-[12px]">Role: Validator (2)</div>
            </div>
          </div>
          <div className="flex gap-3 flex-wrap">
            {hcsResult.txUrl && (
              <a href={hcsResult.txUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-neon-cyan text-xs font-bold hover:underline">
                <ExternalLink size={10} /> HCS Transaction
              </a>
            )}
            {stakeResult?.txHashContract && (
              <a href={`https://hashscan.io/testnet/transaction/${stakeResult.txHashContract}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-neon-green text-xs font-bold hover:underline">
                <ExternalLink size={10} /> Stake TX (Contract)
              </a>
            )}
            {stakeResult?.txHashTransfer && (
              <a href={`https://hashscan.io/testnet/transaction/${stakeResult.txHashTransfer}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-neon-cyan text-xs font-bold hover:underline">
                <ExternalLink size={10} /> MDT Transfer TX
              </a>
            )}
          </div>
          <button onClick={() => onViewChange(ViewState.VALIDATOR_DASHBOARD)}
            className="w-full py-2.5 bg-neon-purple/10 hover:bg-neon-purple/20 border border-neon-purple/40 text-neon-purple font-black text-xs uppercase tracking-widest rounded-xl transition-all">
