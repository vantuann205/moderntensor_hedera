      ...
      <section className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 pb-8 border-b border-white/10 relative">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <span className="px-2 py-0.5 rounded text-[12px] font-bold bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/20 uppercase tracking-widest shadow-[0_0_10px_rgba(0,243,255,0.2)]">Senate Chamber</span>
        ...
        <div className="flex flex-col items-end gap-6 font-display uppercase tracking-widest">
           <div className="glass-panel px-6 py-4 rounded-lg flex items-center gap-8 border border-white/10 bg-black/40">
              <div><p className="text-slate-500 text-[11px] font-black mb-1">Network Power</p><p className="text-2xl font-black text-white">2.8M <span className="text-neon-cyan">M</span></p></div>
              <div className="h-10 w-px bg-white/10"></div>
              <div><p className="text-slate-500 text-[11px] font-black mb-1">Active Proposals</p><p className="text-2xl font-black text-white">4</p></div>
           </div>
        ...
        {proposals.map((p) => (
             <article key={p.id} className={`glass-panel p-6 rounded-2xl border ${p.status === 'active' ? 'border-neon-cyan/50 shadow-[0_0_20px_rgba(0,243,255,0.1)]' : 'border-white/10'} hover:border-white/30 transition-all duration-300 group`}>
                <div className="flex justify-between items-start mb-4">
                    <div className={`px-3 py-1 rounded-full text-[12px] font-black uppercase tracking-widest border ${p.status === 'active' ? 'bg-neon-cyan/10 text-neon-cyan border-neon-cyan/30' : 'bg-neon-green/10 text-neon-green border-neon-green/30'}`}>
                        {p.status === 'active' ? 'Active Voting' : 'Proposal Passed'}
                    </div>
                    <span className="font-mono text-[12px] text-slate-500">REF: {p.id}</span>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-8">
                   <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-[12px] font-mono uppercase tracking-widest text-slate-500">
                   ...
                   <div className="w-full md:w-80 flex flex-col gap-4 font-mono text-[12px] uppercase font-bold">
                        {p.status === 'active' ? (
