...
import { CONTRACTS, STAKING_VAULT_VALIDATOR_ABI, StakeRole } from '@/lib/contracts';
...
export default function ValidatorRegistrationView({ onBack, onViewChange, onRegistered }: Props) {
  ...
  const checkBalance = useCallback(async () => {
    ...
    try {
      ...
      // Check if already staked as validator
      ...
      if (stakeInfo.isActive && Number(stakeInfo.role) === StakeRole.Validator) {
  ...
  const doStake = async (evmAddress: string): Promise<{ txHash: string; txHashTransfer?: string; txHashContract?: string } | null> => {
    ...
    try {
      if (walletType === 'metamask') {
        ...
        const existing = await vault.getStakeInfo(evmAddress);
        if (existing.isActive) { log(`✓ Already staked`); return { txHash: 'already_staked' }; }

        // recordDeposit first
        setStep('deposit');
        log(`Recording deposit...`);
        ...
        log(`Step 3/3: Calling vault.stake(${stake} MDT, Validator) — approve in HashPack...`);
        const params = new ContractFunctionParameters().addUint256(amountRaw.toString()).addUint8(stakeRole);
        const contractId = ContractId.fromEvmAddress(0, 0, CONTRACTS.STAKING_VAULT);
  ...
  const handleRegister = async () => {
    ...
    if (stake < MIN_VALIDATOR_STAKE) { setError(`Minimum stake is ${MIN_VALIDATOR_STAKE} MDT`); return; }
    ...
    try {
      const hcsRes = await fetch('/api/hcs/register', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ role: 'validator', accountId, stakeAmount: stake, skipOnChainStake: true }),
      });
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in max-w-2xl mx-auto">
      ...
      <div className="glass-panel p-4 border border-neon-purple/20 space-y-2 text-[11px] text-slate-400 leading-relaxed">
        ...
        <div className="text-yellow-400 text-[10px] pt-1">⚠ Validator activation requires subnet owner approval — you will see "Pending Approval" after staking.</div>
      </div>
      ...
      {balance && (
        <div className={`glass-panel p-4 flex items-center justify-between border ${balance.hasEnough || alreadyStaked ? 'border-neon-green/20' : 'border-yellow-400/30'}`}>
          ...
          <div className="text-right">
            ...
            <div className="text-[9px] text-slate-500 uppercase tracking-widest">{alreadyStaked ? 'Already staked as Validator' : 'MDT needed'}</div>
          </div>
        </div>
      )}
      ...
      {(step === 'need_faucet' || step === 'faucet_pending') && (
        <div className="glass-panel p-5 border border-yellow-400/30 space-y-3">
          <div className="text-sm font-black text-yellow-400 uppercase tracking-wider">Need MDT</div>
          <div className="text-[11px] text-slate-400">You need <span className="text-white font-bold">{totalNeeded} MDT</span> ({stake} stake + ~{REG_FEE_BUFFER} regFee).</div>
        ...
        <div className="glass-panel p-6 space-y-5">
          ...
          <div className="p-3 bg-neon-purple/5 border border-neon-purple/20 rounded-xl text-[10px] text-slate-400 space-y-0.5">
            ...
            {walletType === 'hashpack' ? (<>
              ...
              <div>3. HashPack: <code className="text-neon-cyan">vault.stake({stake * 1e8}, 2 /* Validator */)</code></div>
            </>) : (<>
              <div>1. Backend: <code className="text-neon-cyan">recordDeposit(userEVM, {totalNeeded} MDT)</code></div>
              <div>2. MetaMask: <code className="text-neon-cyan">vault.stake({stake * 1e8}, 2 /* Validator */)</code></div>
            </>)}
          ...
          {(() => {
            const notReady = !isConnected || stake < MIN_VALIDATOR_STAKE || (!balance?.hasEnough && !alreadyStaked);
            const isDisabled = busy || notReady;
            const btnLabel = busy ? stepLabel
              : !isConnected ? 'Connect wallet first'
              : stake < MIN_VALIDATOR_STAKE ? `Min stake is ${MIN_VALIDATOR_STAKE} MDT`
        ...
        <div className="glass-panel p-6 space-y-4 border border-neon-green/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] text-slate-500 font-mono mt-0.5">Awaiting subnet owner approval to activate</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 font-mono text-xs">
            ...
            <div className="p-3 bg-black/40 rounded-xl border border-white/5 space-y-1">
              ...
              <div className="text-neon-purple text-[10px]">Role: Validator (2)</div>
            </div>
          </div>
