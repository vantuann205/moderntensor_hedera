    ...
    >
      <span className="flex items-center gap-1 whitespace-nowrap">
        ...
        <span className="flex flex-col leading-none opacity-40 group-hover:opacity-100 transition-opacity">
          <span className={`text-[8px] leading-none ${active && sort.dir === 'asc' ? 'text-neon-cyan opacity-100' : ''}`}>▲</span>
          <span className={`text-[8px] leading-none ${active && sort.dir === 'desc' ? 'text-neon-cyan opacity-100' : ''}`}>▼</span>
        </span>
      </span>
    </th>
  );
