...
import { useScores, useTasks } from '@/hooks/useRealData';
...
export default function MinerDetailView({ minerId, onBack }: Props) {
  const { data: allScores, loading: loadingScores } = useScores();
  const { data: allTasks, loading: loadingTasks } = useTasks();
  const loading = loadingScores || loadingTasks;

  // Scores where this miner was scored
  const scores = React.useMemo(() => {
    if (!allScores) return [];
    return allScores
      .filter((s: any) => s.minerId === minerId)
      .sort((a: any, b: any) => {
  ...
  // Enrich scores with task info
  const enriched = React.useMemo(() => {
    return scores.map((s: any) => {
      const task = allTasks?.find((t: any) => t.taskId === s.taskId);
      return { ...s, taskType: task?.taskType, prompt: task?.prompt, rewardAmount: task?.rewardAmount };
    });
  }, [scores, allTasks]);
      ...
      <div className="w-full max-w-[1600px] flex flex-col gap-8">
        ...
        <div className="flex flex-wrap justify-between items-end gap-6 pb-6 border-b border-white/5">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-xl bg-neon-cyan/10 border border-neon-cyan/40 flex items-center justify-center">
              <span className="material-symbols-outlined text-neon-cyan text-3xl">dns</span>
            </div>
            <div>
              <h1 className="text-white text-3xl lg:text-4xl font-black font-display uppercase tracking-tight">{minerId}</h1>
              <p className="text-slate-400 text-sm mt-1">Miner · Hedera HCS Registration Topic</p>
            </div>
          </div>
        ...
        {loading && <div className="text-center py-12 text-slate-500 font-mono">Loading data...</div>}
        ...
        {!loading && stats && (
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              {[
                { label: 'Tasks Scored', val: stats.n, color: 'text-neon-cyan' },
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <div className="glass-panel rounded-xl p-6 border border-white/5">
                <h3 className="text-white font-bold text-sm uppercase tracking-widest mb-4 flex items-center gap-2">
                  <span className="material-symbols-outlined text-neon-cyan text-base">radar</span>
                  Avg Metrics Radar
                </h3>
                <ResponsiveContainer width="100%" height={260}>
                  <RadarChart data={radarData}>
                    ...
                    <Radar dataKey="value" stroke="#00f3ff" fill="#00f3ff" fillOpacity={0.2} strokeWidth={2} />
              ...
              <div className="glass-panel rounded-xl p-6 border border-white/5">
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={barData} barGap={2}>
                    ...
                    <Bar dataKey="confidence" fill="#00ffa3" radius={[3, 3, 0, 0]} name="Conf %" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
                    ...
                    {enriched.map((s: any, idx: number) => (
                      <tr key={idx} className="hover:bg-neon-cyan/5 transition-colors">
                        ...
                        <td className="px-5 py-3 max-w-[140px]">
                          <span className="text-slate-400 truncate block" title={s.prompt}>{s.prompt || '—'}</span>
                        </td>
                        <td className="px-5 py-3 text-neon-purple">{s.validatorId}</td>
        ...
        {!loading && scores.length === 0 && (
          <div className="glass-panel rounded-xl p-12 border border-white/10 text-center">
            ...
            <div className="text-xl text-slate-400">No scored tasks found for this miner</div>
            <div className="text-sm text-slate-500 mt-2">Tasks will appear here after validators score this miner's submissions</div>
          </div>
        )}
      </div>
    </div>
